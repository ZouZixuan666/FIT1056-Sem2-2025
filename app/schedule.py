...
# Move one level up from app/ to pst2/, then into data/
DATA_FILE = os.path.join(BASE_DIR, "..", "data", "msms.json")
DATA_FILE = os.path.abspath(DATA_FILE)  # normalize path
class ScheduleManager:
    ...
    def clear_local_files(self):
        """Deletes msms.json and any student card files in the project folder."""
        files_deleted = 0
    ...
    # Delete all student card files (*.txt with _card naming) inside pst2/
        project_root = os.path.join(BASE_DIR, "..")  # pst2 root
        for file_path in glob.glob(os.path.join(project_root, "*_card.txt")):
            os.remove(file_path)
            files_deleted += 1
 ...
 # ----------------- Teachers -----------------
    ...
    # ----------------- Students -----------------
    def register_student(self, name, instrument):
        if not name.strip() or not instrument.strip():
            return None
        student = StudentUser(self.next_student_id, name.strip())
        student.enrolled_course_ids.append(instrument)
        self.students.append(student)
        self.next_student_id += 1
        self._save_data()
        return student

    def enrol_student(self, student_id, instrument):
        student = self.find_student(student_id)
        if not student:
            return False
        if instrument not in student.enrolled_course_ids:
            student.enrolled_course_ids.append(instrument)
            self._save_data()
            return True
        return False
    ...
    def print_student_card(self, student_id):
        ...
        with open(filename, "w") as f:
            ...
            f.write(f"Enrolled In: {', '.join(student.enrolled_course_ids)}\n")
        return filename
    ...
    def find_course_by_id(self, course_id: int) -> Course | None:
        """Return a Course by ID or None if not found."""
